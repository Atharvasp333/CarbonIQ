    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute("""
            INSERT INTO aws_credentials
                (user_id, access_key, secret_key, region, bucket_name, file_key, verified, verified_at, updated_at)
        ...
        """, user_id, req.access_key, req.secret_key, req.region,
             req.bucket_name, req.file_key or None, now)
...
async def get_creds(req: AWSCredsEmailRequest):
    """Get saved AWS credentials for a user (full keys returned for sync)."""
    if not req.email:
        raise HTTPException(status_code=400, detail="email required")
