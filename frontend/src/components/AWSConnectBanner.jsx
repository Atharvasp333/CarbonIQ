...
import { fetchAWSData, saveAWSCredentials, getAWSCredentials, deleteAWSCredentials } from '../api/client';
...
export default function AWSConnectBanner({ onDataLoaded }) {
  ...
  useEffect(() => {
    ...
    getAWSCredentials(user.email)
      .then(d => setStatus(d.connected ? d : false))
      .catch(() => setStatus(false));
  }, [user?.email]);

  // Close region suggestions on outside click
  useEffect(() => {
    const handler = (e) => {
  ...
  const handleSave = async (e) => {
    ...
    setSaving(true);
    try {
      toast.loading('Connecting to S3 & running pipeline… (may take 30–60s)', { id: 'save' });

      // Fetch from S3 and run through the full pipeline
      ...
      // Save credentials to NeonDB (keyed by email, no JWT needed)
      ...
      const fresh = await getAWSCredentials(user.email);
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      toast.success(kg ? `Synced! ${kg} kg CO₂ calculated` : 'S3 data synced!');
      onDataLoaded?.(data);
    } catch (err) {
      toast.dismiss('sync');
  ...
  const handleDisconnect = async () => {
    ...
    await deleteAWSCredentials(user.email).catch(() => {});
    setStatus(false);
      ...
      <button onClick={() => setShowForm(v => !v)}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">Link your S3 bucket — credentials saved to your account, works on all devices</p>
          </div>
        </div>
